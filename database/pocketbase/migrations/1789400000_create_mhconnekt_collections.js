...
// MH Connekt is kept separate from Taiga so a customer can connect both
// services to the same WhatsApp number without either session overwriting the
// other. All fields are server-only; tokens are AES-GCM encrypted before save.
migrate((app) => {
  ...
  for (const name of ['mhconnekt_sessions', 'mhconnekt_connections']) {
