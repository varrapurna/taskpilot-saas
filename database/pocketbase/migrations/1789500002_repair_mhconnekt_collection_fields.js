...
// The original MH collection migration shares its numeric prefix with an
// unrelated migration. On affected PocketBase databases the collections were
// created without their custom fields. Add every missing field in place; do
// not delete existing records or replace either collection.
migrate((app) => {
  ...
  for (const definition of collections) {
    ...
    for (const index of definition.indexes) {
      if (!indexes.some((current) => current.includes(index.match(/INDEX (\S+)/)[1]))) indexes.push(index);
    }
    collection.indexes = indexes;
    app.save(collection);
  }
}, () => {
  // Repairing production schema must never remove customer connection data.
});
